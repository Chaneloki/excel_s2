...
const SE_AUDIO_DIR := "res://assets/se/"
const SE_AUDIO_EXTENSION := ".mp3"
...
# 切換BGM時淡出/淡入各自的時間；BGM_FADE_OUT_VOLUME_DB是淡出底點，
# 不用-80dB（幾乎無聲但Tween要跑很久才感覺到變化），-24dB已經聽起來
# 接近無聲，淡出/淡入時間可以縮短，銜接感更好。
const BGM_FADE_DURATION := 0.6
# 對白資料沒填"bg_zoom_duration"時的預設值，背景縮放/位移要慢、要撐過
# 好幾句對白才看得出差異，不能跟shake/punch_zoom那種瞬間特效一樣短。
const BG_ZOOM_DEFAULT_DURATION := 10.0
# "bg_pos"關鍵字 -> 方向係數(fx,fy)，每個成分只會是-1/0/1，跟v2舊作
# ui_v2_story.js的posFactors對照表完全一致，沿用同一套字詞方便比照舊
# 劇本素材。
const BG_POS_FACTORS := {
	"center": Vector2(0, 0),
	"left": Vector2(1, 0),
	"right": Vector2(-1, 0),
	"top": Vector2(0, 1),
	"bottom": Vector2(0, -1),
	"left bottom": Vector2(1, -1),
	"right bottom": Vector2(-1, -1),
	"left top": Vector2(1, 1),
	"right top": Vector2(-1, 1),
	"center bottom": Vector2(0, -1),
}
const BGM_FADE_OUT_VOLUME_DB := -24.0
...
# BGM id -> 圖檔路徑，_show_line()依對白資料的"bgm"欄位切換背景音樂。
var bgm_lookup: Dictionary = {}
...
# 目前播放中的BGM id，邏輯跟current_background_id一樣：避免同一首BGM
# 被重複觸發從頭播放，空字串代表還沒有任何BGM開始播放過。
var current_bgm_id := ""
# 「sticky鏡頭」目前的縮放倍率/位移方向，只有對白資料明確帶
# "bg_zoom"/"bg_pos"才會更新，其餘句子沿用上一次的值（見_show_line()）。
var current_bg_zoom := 1.0
var current_bg_fx := 0.0
var current_bg_fy := 0.0
...
var objective_panel_collapsed := true
...
var background_base_size := Vector2.ZERO  # background_rect在offset全部=0（沒放大）時的原始尺寸，ken_burns()算縮放/位移目標要用這個基準，量一次存起來
var bgm_player: AudioStreamPlayer  # 背景音樂播放器，_switch_bgm()依對白資料的"bgm"欄位換曲目
var bgm_tween: Tween  # 記錄換曲淡出/淡入，停止BGM時要先終止，避免舊Tween稍後又把音樂啟動
var se_players_container: Node  # 每個同時播放的SE各用一個AudioStreamPlayer，容器統一管理生命週期
var active_se_players: Array[AudioStreamPlayer] = []
var cg_image_rect: TextureRect    # CG插畫實際內容，疊在cg_layer佔位色之上；沒有對應素材時維持隱藏，底下的佔位色+文字會照常顯示
var cg_base_size := Vector2.ZERO  # cg_image_rect在offset全部為0時的原始尺寸，讓CG可沿用背景的Ken Burns運鏡
...
func _load_case_data() -> void:
	...
	bgm_lookup = case_data.get("bgm_tracks", {})
...
func _build_ui() -> void:
	...
	_build_bgm_player()          # 8. 背景音樂播放器，沒有視覺層級可言，順序無所謂
	_build_se_player()           # 9. 單句音效播放器，音量接設定畫面的「音效音量」
...
# 背景音樂播放器：跟bgm_volume滑桿的_set_bgm_volume()共用同一個節點，
# 滑桿原本只改變一個沒人讀的數字，現在真的會改變這個播放器的音量。
...
# 依bgm_lookup換背景音樂：相同曲目不重新播放；空字串或不存在的id會停止
# 目前BGM，避免劇本要求的曲目缺失時錯誤沿播上一首。正常換曲使用Tween
# 淡出目前音量、換成新AudioStream、再淡入，避免硬切。
func _switch_bgm(bgm_id: String) -> void:
	if bgm_id == "" or not bgm_lookup.has(bgm_id):
		if bgm_id != "":
			push_warning("Chapter1Dialogue: 找不到BGM id " + bgm_id + "，已停止目前BGM。")
		_stop_bgm()
		return
	...
	var target_volume_db: float = linear_to_db(bgm_volume)

	if not bgm_player.playing:
		bgm_player.stream = load(bgm_lookup[bgm_id])
		bgm_player.volume_db = target_volume_db
		bgm_player.play()
		return

	bgm_tween = create_tween()
	bgm_tween.tween_property(bgm_player, "volume_db", BGM_FADE_OUT_VOLUME_DB, BGM_FADE_DURATION)
	bgm_tween.tween_callback(func() -> void:
		bgm_player.stream = load(bgm_lookup[bgm_id])
		bgm_player.play()
	)
	bgm_tween.tween_property(bgm_player, "volume_db", target_volume_db, BGM_FADE_DURATION)
...
# SE容器：跟BGM分開，播放SE不會中斷背景音樂。每個SE在播放時動態建立
# 自己的AudioStreamPlayer，因此同一句的多個音效可以真正同時播放。
func _build_se_player() -> void:
	se_players_container = Node.new()
	se_players_container.name = "SePlayers"
	add_child(se_players_container)


# se可填單一字串或字串陣列；陣列內每個音效會在同一畫面更新中啟動，
# 達到同時播放。舊的單一字串格式繼續相容。
func _play_se(se_value: Variant) -> void:
	var se_ids: Array = se_value if typeof(se_value) == TYPE_ARRAY else [se_value]
	for value in se_ids:
		_play_single_se(str(value))


func _play_single_se(se_id: String) -> void:
	if se_id == "":
		return

	var se_path := SE_AUDIO_DIR + se_id + SE_AUDIO_EXTENSION
	if not ResourceLoader.exists(se_path):
		push_warning("Chapter1Dialogue: 找不到SE音檔 " + se_path)
		return

	var player := AudioStreamPlayer.new()
	player.name = "SePlayer_" + se_id
	player.stream = load(se_path)
	player.volume_db = linear_to_db(sfx_volume)
	player.finished.connect(_on_se_player_finished.bind(player))
	se_players_container.add_child(player)
	active_se_players.append(player)
	player.play()


func _on_se_player_finished(player: AudioStreamPlayer) -> void:
	active_se_players.erase(player)
	player.queue_free()
...
func _build_background() -> void:
	...
	background_base_size = background.size
...
func _build_objective_panel() -> void:
	...
	# 開場還沒有任何案件目標時，面板展開也是空的，預設收合比較合理，
	# 玩家自己點標題列展開即可，呼應objective_panel_collapsed預設true。
	_set_objective_panel_collapsed(objective_panel_collapsed)
...
# 對白播放區
# ------------------------------
# Chapter 1目前是純Story Dialogue demo，只顯示dialogue/narration/system
# 三種故事內容。hotspot與excel_stage_trigger仍保留在案件資料中供未來
# 整合使用，但這個場景會略過，不建立Map Walker或Excel Solver。
func _is_story_line(line: Dictionary) -> bool:
	return line.get("type", "") in ["dialogue", "narration", "system"]


# 從指定位置開始尋找下一句真正的故事內容；搜尋最多繞完整份劇本一次，
# 避免資料全是結構標記時形成無限循環。
func _find_story_line_index(start_index: int) -> int:
	if dialogue_lines.is_empty():
		return -1

	var candidate := posmod(start_index, dialogue_lines.size())
	for _step in range(dialogue_lines.size()):
		if _is_story_line(dialogue_lines[candidate]):
			return candidate
		candidate = (candidate + 1) % dialogue_lines.size()
	return -1
...
func _show_line(index: int) -> void:
	var story_index := _find_story_line_index(index)
	if story_index < 0:
		push_warning("Chapter1Dialogue: 劇本內沒有可播放的Story Dialogue內容。")
		return
	current_line_index = story_index

	# index可能正好指向已略過的hotspot/excel_stage_trigger，必須改讀實際
	# 找到的story_index，否則畫面索引與播放內容會不同步。
	var raw_line: Dictionary = dialogue_lines[story_index]
	...
	if raw_line.has("bgm"):
		_switch_bgm(raw_line["bgm"])

	if raw_line.has("se"):
		_play_se(raw_line["se"])
	...
	# 普通背景與CG共用同一套逐句運鏡欄位：bg_zoom、bg_pos、
	# bg_zoom_duration。要先完成上面的CG模式判斷及貼圖切換，才能把動畫
	# 套到正確的TextureRect；否則CG句會錯誤地繼續移動底下的普通背景。
	# 沒填運鏡欄位時不建立新Tween，上一段仍在播放的動畫會自然跑完。
	if raw_line.has("bg_zoom") or raw_line.has("bg_pos"):
		# sticky鏡頭狀態：只更新這句有填的部分，另一部分沿用上一次設定。
		if raw_line.has("bg_zoom"):
			current_bg_zoom = raw_line["bg_zoom"]
		if raw_line.has("bg_pos"):
			var factors: Vector2 = BG_POS_FACTORS.get(raw_line["bg_pos"], Vector2.ZERO)
			current_bg_fx = factors.x
			current_bg_fy = factors.y

		var pan_factor := Vector2(current_bg_fx, current_bg_fy)
		var duration: float = raw_line.get("bg_zoom_duration", BG_ZOOM_DEFAULT_DURATION)
		var camera_target: TextureRect = cg_image_rect if is_cg_scene else background_rect
		var camera_base_size: Vector2 = cg_base_size if is_cg_scene else background_base_size
		dialogue_effects.ken_burns(camera_target, camera_base_size, current_bg_zoom, pan_factor, duration)
...
func _set_sfx_volume(value: float) -> void:
	sfx_volume = value
	for player in active_se_players:
